...
func (s *Syncer) FullSync(ctx context.Context, repo *store.Repository, actor string) (*SyncResult, error) {
	...
	configCount, configDeleted, configErr := s.SyncConfigFromDir(ctx, tmpDir, repo.ConfigPath, repo.ID, actor)
	if configErr != nil {
...
func (s *Syncer) SyncConfigFromDir(ctx context.Context, dir, configPath, repoID, actor string) (int, int, error) {
	if configPath == "" {
		return 0, 0, nil
	}
