...
- **Where:** `internal/gitops/sync.go:405-471` (`storeSecret` → `PutSecret`), `internal/store/secrets.go:43-79` (`PutSecret` always auto-increments `version`), `internal/gitops/reconcile.go:30-44` (default 5-minute `FullSync` of all repos), `SyncFromDir` re-stores every file each run.
- **Related follow-on (bytepunx/signet#55):** this dedup fix stopped the storage bloat, but `storeSecret`'s unchanged-skip and its real-write path both returned a plain `error`, so `SyncFromDir`/`SyncFromPush` couldn't tell "wrote" from "skipped as unchanged" and counted/notified both identically. That meant every reconciliation tick still fired a real `WatchServiceBundle` bundle-changed notification for every unchanged secret, which any consumer using the coordinated-restart pattern (`docs/restart-lock.md`) correctly treated as a real change — a permanent restart loop, observed in production. Fixed by having `storeSecret` return `(wrote bool, err error)` so callers only count/notify on an actual write, plus a remote-HEAD-SHA short-circuit in `reconcileAll` (`Syncer.ResolveHeadSHA`) so a no-op tick skips the clone+walk entirely. Tests: `internal/gitops/sync_test.go`'s `TestSyncFromDir_NoNotifyOrAddedOnUnchangedResync`.
